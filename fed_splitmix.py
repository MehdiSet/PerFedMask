...
# federated
from federated.learning import train_slimmable, test, personalization_slimmable
# model and data
...
def render_run_name(args, exp_folder):
    ...
    if args.model == 'default':
        args.model = {'Digits': 'ens_digit', 'Cifar10': 'ens_preresnet18', 'Cifar100': 'ens_mobile', 'DomainNet': 'ens_alex'}[args.data]
...
def get_model_fh(data, model, atom_slim_ratio):
    ...
    if data == 'Digits':
        ...
        if model in ['ens_mobile']:
            from nets.slimNets import MobileNetCifar
            ModelClass = lambda **kwargs: EnsembleNet(
                base_net=MobileNetCifar, atom_slim_ratio=atom_slim_ratio,
                rescale_init=args.rescale_init, rescale_layer=args.rescale_layer, **kwargs)
        else:
            raise ValueError(f"Invalid model: {model}")
    else:
...
def fed_test(fed, running_model, train_loaders, val_loaders, global_lr, verbose, val_mix_model=None):
    mark = 's' 
    ...
    for client_idx in range(fed.client_num):
        ...
        for i_slim_ratio, slim_ratio in enumerate(fed.val_slim_ratios):
            ...
            val_model = copy.deepcopy(val_mix_model)
            ...
            if args.test:               
                ...
                val_loss, val_acc = personalization_slimmable(val_model, train_loaders[client_idx], val_loaders[client_idx],
...
if __name__ == '__main__':
    ...
    parser.add_argument('--algorithm', type=str, default='splitMix', help='algorithm name') 
    ...
    for userIdx in range(fed.client_sampler.tot()):
        ...
        in sorted(zip(slim_ratios, slim_shifts, out_slim_shifts), reverse=False,
            ...
            temp_model.switch_slim_mode(slim_ratio, slim_bias_idx=in_slim_shift, out_slim_bias_idx=out_slim_shift)
    ...
    for a_iter in range(start_epoch, args.iters):
        ...
        for client_idx in fed.client_sampler.iter():
            ...
            local_iters = mean_batch_iters * local_iter if args.partition_mode != 'uni' \
        ...
        val_acc_list, val_loss, val_acc_list_bp, val_loss_bp = fed_test(fed, running_model, train_loaders, val_loaders, 
                                                    global_lr, args.verbose, val_mix_model=val_mix_model)
        ...
        if args.adv_lmbd > 0:
            ...
            val_racc_list, val_rloss = fed_test(
                fed, running_model, args.verbose, val_mix_model=val_mix_model)
